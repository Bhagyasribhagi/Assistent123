...
export default function DragAndDrop({ listItems, heading }) {
  ...
  const handleDrop = (event) => {
    ...
    if (item) {
      ...
      ).findIndex((element) => element.contains(event.target));
  ...
  return (
    <div className="flex w-full p-5 flex-col gap-1">
      <div className="p-4 items-center justify-center flex flex-col lg:p-10 mt-5 bg-gray-300 rounded-lg shadow-lg">
        <h1 class="font-bold text-transparent my-3 text-xl items-center justify-center flex  bg-clip-text bg-gradient-to-r from-green-500 via-orange-400 to-red-300">
          {heading}
        </h1>
        <ul
